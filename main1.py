...
Analyze the menu photo and respond ONLY with a valid JSON object (no markdown, no backticks, no preamble).
...
{{
  ...
  "items": [
    {{
      ...
      "recommendReason": "why this suits the user (only if recommended=true, else null)"
    }}
  ],
...
Rules:
- Translate menu names naturally to {lang_label}
- Mark recommended:true for 2-4 items matching user preferences
- Set allergyWarning for items conflicting with user's allergies
- Keep descriptions warm and traveler-friendly"""
...
""", unsafe_allow_html=True)
                ...
                st.error("메뉴 파싱에 실패했습니다. 사진을 다시 시도해주세요.")
